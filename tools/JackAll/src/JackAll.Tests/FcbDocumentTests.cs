...
public class FcbDocumentTests
{
    ...
    [Fact]
    public void EncodedSize_matches_what_Serialize_actually_emits()
    {
        var root = new FcbObject { TypeHash = 0xCAFEBABE };
        ...
        childB.Values.Add(0x00000003, new byte[300]); // large enough to need the 0xFF + u32 count form
        var grandchild = new FcbObject { TypeHash = 0x33333333 };
        grandchild.Values.Add(0x00000004, [0xFF]);
        childB.Children.Add(grandchild);
        root.Children.Add(childA);
        root.Children.Add(childB);

        Assert.Equal(TestSupport.FullyExpandedFcbSize(root), FcbDocument.EncodedSize(root));
        Assert.Equal(TestSupport.FullyExpandedFcbSize(childB), FcbDocument.EncodedSize(childB));
    }
