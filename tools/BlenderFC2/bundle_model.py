...
#   python bundle_model.py <character.xbg> --clip <reload.mab>
#
# The root is the directory an archive was extracted to, the one holding
# `graphics`; it is found from the model's own path when not given. A clip
# folds in whatever it attaches to the model, so the bundle can play it.
...
from fc2fmt.assets import InstallAssets, find_named, find_root
from fc2fmt.bundle import EXTENSION, Bundle
from fc2fmt.mab import MabFile


def clip_models(source, clip_path):
    """The models a clip attaches, as game paths."""
    clip = MabFile.parse(open(clip_path, "rb").read())
    found = []
    for participant in clip.participants():
        if not (participant.parent and participant.is_primary):
            continue
        candidates = find_named(source, participant.name)
        if candidates:
            found.append(candidates[0])
        else:
            print("no model named %s" % participant.name, file=sys.stderr)
    return found
...
def main(argv=None):
    ...
    parser.add_argument("--clip", action="append", default=[],
                        help="also pack whatever this .mab attaches to the model")
    ...
    source = InstallAssets(root)
    also = []
    for clip in args.clip:
        for model in clip_models(source, clip):
            if model not in also:
                also.append(model)
    bundle = Bundle.build(os.path.relpath(args.model, root), source, also)
