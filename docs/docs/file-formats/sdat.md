...
:::info[Verified via reverse engineering — corrects a community claim]
Traced live via GhidraMCP against **`FarCry2_server`** (the Linux dedicated-server ELF, not
`Dunia.dll`), whose fuller symbol/type table keeps real class names like `CSector`,
`SSectorDataChunk` and `CSceneTerrainSectorPackedData`. Covers the on-disk byte layout of
world-sector `sdN.sdat` files, the per-8×8-grid terrain sectors catalogued at a black-box level in
[Engine Theory](../modding/engine-theory.md).
:::

## Not a bare 513×513 heightmap

A Discord-derived community claim holds that "`.sdat` is a pure 513×513 `u16` heightmap grid, no
header at all." **That is not what this binary does.** The real format is a generic chunked container with a
...
- **Reader**: `CSector::Load(uchar*)`. Reads back exactly what the writer produced — every
...
height_m   = (float)height_u16 * 0.0078125      // 1/128
```
...
`PreparePackedDataForExport`'s own 64-iteration mask loops (`local_290 != 0x40`) run over the same
64-wide base grid.

The height scale `0.0078125` (1/128) appears as an inline constant in `CTerrain::GetZ`,
`CTerrain::GetSectorHeightFloat`, `CSector::GetZApr` and `CSector::ComputeMinMaxZ`. It matches flat
editor terrain, which stores 2048 while objects rest at z=16.
...
The layer index is the top three bits: `CSector::GetHole`, `CTerrain::IsSectorEditorHole` and
`CSector::GetSurfaceType` all evaluate `byte >> 5`, giving a range of 0–7 with **7 reserved to mean
hole**. The triangle-index builders consume the same byte.
...
examined, implying this container may be shared with other platforms' builds). Whether
[`.fcb`](./fcb.md) reuses this same class is not cross-checked — its header as documented there
doesn't obviously match this 20-byte shape, so likely a separate format.

## Sibling per-sector files: `.srl` and `.zsr`

Every world sector has two companion files alongside its `.sdat` (per the hash list, 14,964 of each
across the whole install — `generated\worldsectors\sectorN.srl` and
`generated\worldsectors\zonesectorN.zsr`, versus `generated\sdat\sdN.sdat` in a sibling folder, not
literally co-located). Unlike this page's chunked container, both are raw fixed-size per-sector
memory dumps with no header at all — see [`.srl`/`.zsr`](./srl-zsr.md) for the full writeup.
...
[environment presets](../modding/environment-presets.md)) with a handful of literal, immediately
usable values:
...
table shows are the `Mountain_Rock_X` and `Mountain_Rock_Y` entries of one biome. Layer names ending
`_X`/`_Y`/`_Z` are one texture projected down different axes; the editor palette carries them as a
single entry with `ProjectionX`/`ProjectionY` attributes, so they collapse when mapping to it.
...
A correlation that high means the bake carries the sun's angle. The correlation cannot separate a
full lightmap from a self-shadow term, though, because slopes facing away from the sun are both dark
*and* self-shadowed. What settles it is the engine's own terrain pixel shader
(`shadersobj/engine/shaders/obj10`, RDEF names intact): its shadow map **scales the light colour
while `saturate(N·L)` still shades the surface** —
...
The best-fit sun sits near the horizon in the **west**. A direction of `(0.4, 0.3, 0.85)`, roughly
the opposite, scores **-0.50**; anti-correlated shading and the bake darken each other everywhere
instead of agreeing, which makes textured terrain look muddy.
...
The channels do not sum to 255 at every texel, and the near-black population is exactly the cliff
faces: a black mask texel means solid rock, not absence of data. Treating the mask as three
normalised weights leaves those texels with no layer at all, which draws as flat grey patches on
hillsides.
...
relative to each sector's own `DetailTexMask` layers and legitimately jump at a boundary.

## Unknowns

- Bit 4 of the `+0x3` flags byte (bits 3..0 are
  [the quad mask](#the-low-nibble-is-the-quad-mask)) is unclaimed by any traced reader or writer; it
  is set in roughly 10% of campaign cells.
...
- `SSectorDataChunk+0x010` and `+0x01C`'s exact semantics — both round-trip correctly but their
  purpose is not identified beyond "some per-sector field" / "constant 1".
