...
The Ogg-backed layout is derived from direct byte-analysis of 54 real music `.sbao` files (from
...
`Dunia.dll` — full writeup on [the `.spk` page](./spk.md#the-audio-codecs-ogg-vorbis-and-ima-adpcm),
since it's the same underlying DARE data reached through a different container path.
:::

`.sbao` = "sound binary audio object," a standalone (non-bank) DARE resource file — the same
"atomic"/"streamed" object model documented on [`.spk`](./spk.md), saved to its own file
(`<id:08x>.sbao` or `.bao`) instead of packed into a bank. Two layouts exist.

## Long audio (music, dialogue): `[40-byte header][verbatim Ogg Vorbis bitstream]`

An Ogg Vorbis stream in a thin wrapper. The header is byte-identical across every music file except a
16-byte field at `0x08`:
...
- **Encode/replace** = `original_header[:40] + new_ogg`. No size field, no rate field, and no checksum
  to recompute — reusing the original 40-byte header (GUID included) is safe because the GUID isn't
  derived from the content.
- A robust reader should still read the offset at `0x04` rather than hardcode 40 (all retail files use
  40, but the field exists for a reason).
...
verified by decoding real payloads end-to-end into playable audio. Full writeup — table addresses, the
two decoder functions, the 28-byte per-stream header layout, and the verification — is on [the `.spk`
page](./spk.md#the-audio-codecs-ogg-vorbis-and-ima-adpcm).

This is a different, unrelated codec family from Ubisoft's older in-house "Ubi Sound Tools" ADPCM
dialects (`ubi_v3`/`v5`/`v6`/interleaved, decodable by the third-party tool `Ubitunedec`, used in older
...
Sound objects are requested through the engine's VFS by relative path (observed in
`modpatcher.log`: `[VFS] passthrough SoundBinary\...`), so a repacked file dropped at
`Data_Win32\Loose\soundbinary\004b177b.sbao` is overridden by ModPatcher's Phase 1 `VFS_ResolvePath`
hook (and, if requested by hash, the Phase 2 `ArchiveEntry_FindAndOpen` hook) — the identical resolver
`.spk` bank loading uses. See [archives](./archives-fat-dat.md).
