...
Every one of the 37 registrations follows this exact shape — effectively the developers' own in-source
API documentation for the language, compiled straight into the shipped binary and normally only
visible one function at a time through the editor's script-help UI. Confirmed struct layout:
...
apply-to-world steps. Statement syntax is visible directly in the prototype strings (e.g. `map =
GenerateCircle(x, y, radius, hardness, falloff, opacity);`) — assignment, C-style call syntax,
...
pregeneration feature noted in [Engine Theory](../modding/engine-theory.md); the general scripted path
is a strong candidate for how the other biome presets (Savannah, Jungle) are implemented, if they
aren't equally hardcoded native functions.
...
37 functions total: 2 random, 2 lifecycle, 11 generators, 15 operators, 7 world-application calls.
