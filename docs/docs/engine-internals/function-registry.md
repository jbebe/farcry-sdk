...
|---|---|---|
| `"incHB"` | `0x1065aea0` | Health-bar-looking float math (clamp/compare) — a **live gameplay tick**, not dead code, despite being a no-op stub (`SaveGame`/`incHB` shared) on the exe side. **Tested live in-game** (`reverse/patch_incHB.py`): the value is passed **by raw bits through EAX**, not FPU/ST(0) convention — confirmed via the caller's own disassembly (`MOVSS`/`COMISS` around the call, no `CVTSI2SS`/`FILD`). A handler using `FLD` crashes the game (an unpopped x87 stack push on every call overflows the 8-deep FPU stack within seconds); a pure `MOV EAX,[ESP+4]` echo is stable and restores the real time-driven value into the downstream comparison. What bit `0x8` of the flags value passed to `FUN_104cfc90` actually controls is unconfirmed — it makes no visible in-game difference, so the "heartbeat" reading of `"HB"` is unconfirmed, not ruled out. |
...
- Bare `"SaveGame"` (as opposed to its alias `"incHB"`, which does have a confirmed call site) has no
  call site among those surveyed — status less certain than `SetDefaultTimeOut`'s, since a call site could
...
this registry. It also explains why most exe-side handlers are harmless dead stubs in retail: the
call site tolerates them not existing at all.
