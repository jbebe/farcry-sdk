...
| `worlds.fat\world{1,2}\generated\entitylibrary{,_full}.fcb` | Per-map (Leboa/Bowa) entity definitions. **Load-order gotcha**: despite the "full" name suggesting primacy, the game loads `entitylibrary_full.fcb` first and `entitylibrary.fcb` second in client/singleplayer mode — so `entitylibrary.fcb` wins on any overlapping entity. Put your edits there. World2 is a near-fully duplicated structure from world1, not shared/parameterized. |
