...
from the [OpenWorldGames forum](https://www.openworldgames.org/owg/forums/index.php?board=169.0)
(see [Getting Started](./getting-started.md)). ~22 Nexus/ModDB mods are cross-posted to both — those
are listed once with both links.

## Technical facts from mod descriptions
- **Archive format confirmed**: multiple mod descriptions reference `patch.dat` / `patch.fat` as the pair of files that hold the game's packed assets — "Far Cry 2 Rewards" explicitly warns Desura users to back these up before installing.
...
- **A mod-merging tool exists**: "Far Cry 2 Universal Patcher" claims to combine arbitrary mods with any master mod "without any prior knowledge of the modding tools".
...
- [An Almost Complete Guide to Far Cry 2 Modding](https://www.nexusmods.com/farcry2/mods/299) ([ModDB mirror](https://www.moddb.com/mods/an-almost-complete-guide-to-far-cry-2-modding)) — Boggalog. The closest thing to an SDK doc. Its live Google Doc (shared on Discord, `🔨-fc2-modding`, by "Hunter" — see [sources](./sources.md)) is `https://docs.google.com/document/d/1ozhN9s_4puzSXVYs12ZAOayyL036hgQuFCVS_jSXbd0/edit`; the Discord embed preview shows its table of contents: Contributors · Quick Steps (editing an existing mod) · Modding Tools · Packing/unpacking · XML Editing · XML Decoding · Hex editing · Texture conversion · Texture editing · File Management · .fat and .dat files · Unpacking .fat... — the comprehensive walkthrough its Nexus description promises. It is on this site as [the Almost Complete Guide](./guide).
- **[Far Cry 2 Fortunes Pack installer](https://www.moddb.com/games/far-cry-2/downloads/far-cry-2-fortunes-pack)** (ModDB, surfaced via Discord) — not a mod, an **environment-setup utility**: installs the Fortune's Pack expansion content (extra maps/weapons/vehicles) that Steam/Uplay/GOG copies already bundle but disc/ISO installs lack. Several community map-editor mods and multiplayer maps explicitly require Fortune's Pack to be installed to load correctly, so a disc-based install needs it before a map or mod can be judged broken.
...
- [Ultimate Map Editor](https://www.moddb.com/mods/ultimate-map-editor) — ModDB only. "Combines the already existing custom Map Editors into 1, expanded with 100s of new objects."
...
- **Gabor's "FC2 editor mod"** (not publicly released; shared ad hoc within the "Far Cry 2 Multiplayer" Discord's `map-editor`/`modding` channels) — imports **1,700+ objects from Avatar: The Game (2009)** into the FC2 map editor's object library as static decoration (vehicle animations don't port due to third-person-vs-first-person rig incompatibility — see [Engine Theory](./engine-theory.md)). The community's investigation of the FC2/Avatar shared Dunia engine starts from this mod.
...
- [Calls from the Mountain](https://www.moddb.com/mods/calls-from-the-mountain) — ModDB only. Built with FC2's first-version map tools.
...
- [Rare Malaria Animation](https://www.moddb.com/mods/rare-malaria-animation) — ModDB only, different upload/author — likely the same underlying trick as Boggalog's rather than a second independent source.
...
- "Merc War - Mercs vs Mercs, real firefight" — Art Blade (OWG, savegame-based scenario referenced in the malaria-removal thread; no direct download link, available through OWG's Downloads section).
...
2. **Far Cry 2 Universal Patcher** (ModDB) — for its merge logic.
3. **An Almost Complete Guide to Far Cry 2 Modding** (Boggalog) — closest thing to an SDK doc, on this site as [the Almost Complete Guide](./guide) (7,992 lines). These pages do not summarize it line by line; search it directly when a specific modding question comes up.
4. **Hunter's Far Cry 2 Update** (ModDB) and **Hunter's Gibbed-style tools** — Hunter appears repeatedly as a foundational tool/patch author across both sites.
