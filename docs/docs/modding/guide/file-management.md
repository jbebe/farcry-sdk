...
2. Drag either the .fat or .dat file onto "Gibbed.Dunia.Unpack.exe". This will unpack both files and create a folder called "EXAMPLE_unpack", the ‘EXAMPLE’ section will match the name of the file you unpacked.
...
1. Drag the "EXAMPLE_unpack" folder onto "Gibbed.Dunia.Pack.exe" and this will create two new files: “EXAMPLE_unpack.dat" and "EXAMPLE_unpack.fat".
...
3. Open “EXAMPLE.xml”. You can see that each .xml file in the “EXAMPLE” folder is listed in the format of “\<object external="38_XXXXXX.xml" /\>”. Copy the last line with a filename and paste it directly below, so you have two identical lines that contain a filename.
