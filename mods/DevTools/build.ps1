<#
.SYNOPSIS
    Builds DevTools.dll with the correct 32-bit toolchain, and optionally installs it into the game.
...
$OutputDll = Join-Path $BuildDir "DevTools.dll"
...
if ($Install) {
    ...
    Copy-Item $OutputDll (Join-Path $PluginDir "DevTools.dll") -Force
    Write-Host "Installed: $(Join-Path $PluginDir 'DevTools.dll')" -ForegroundColor Green
}
